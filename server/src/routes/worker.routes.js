...
/** Resolve employee_id for a user */
async function resolveEmployeeId(userId) {
  ...
  if (!user) return { employeeId: null, userRow: null };
...
router.get('/summary', requireAuth, requireRole('worker'), async (req, res) => {
  try {
    ...
    const rows = await q(pool, `
      SELECT pe.id, pe.entry_date, l.lot_number, lo.op_name,
             lo.rate_per_piece, pe.pcs, (pe.pcs * lo.rate_per_piece) AS income
...
router.get('/monthly', requireAuth, requireRole('worker'), async (req, res) => {
  try {
    ...
    const rows = await q(pool, `
      SELECT DATE_FORMAT(pe.entry_date, '%Y-%m') AS month,
