...
router.get('/summary', requireAuth, requireRole('admin', 'supervisor', 'incharge'), async (_req, res) => {
  try {
    const [totalsRow] = await q(pool, 'SELECT COUNT(*) AS employees FROM employees');
    const [lotsCountRow] = await q(pool, 'SELECT COUNT(*) AS lots FROM lots');
    ...
    const lotProgress = await q(pool, `
      SELECT
        l.id, l.lot_number, l.target_qty, COALESCE(SUM(pe.pcs),0) AS produced
      FROM lots l
