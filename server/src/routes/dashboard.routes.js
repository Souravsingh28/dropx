...
router.get('/summary', auth, requireRole('admin','supervisor','incharge'), async (req, res) => {
  ...
  const recent = await q(pool, `SELECT pe.id, pe.entry_date, e.name, l.lot_number, lo.op_name, pe.pcs
                                FROM production_entries pe
                                JOIN employees e ON e.id = pe.employee_id
                                JOIN lots l ON l.id = pe.lot_id
                                JOIN lot_operations lo ON lo.id = pe.operation_id
                                ORDER BY pe.entry_date DESC, pe.id DESC LIMIT 10`);
